...
use crate::bfrt_proto::{data_field, DataField, Entity, entity, key_field, KeyField, TableData, TableKey, Update};
...
impl BFRTTableObject {
            ...
            for entry in m {
                ...
                let match_type = match &entry.1 {
                    MatchValue::ExactValue { bytes } => {
                        MatchType::Exact(key_field::Exact { value: bytes.to_vec().convert(key_name, key_width)? })
                    }
                    MatchValue::RangeValue { lower_bytes, higher_bytes } => {
                        MatchType::Range(key_field::Range { low: lower_bytes.to_vec().convert(key_name, key_width)?, high: higher_bytes.to_vec().convert(key_name, key_width)? })
                    }
                    MatchValue::LPM { bytes, prefix_length } => {
                        MatchType::Lpm(key_field::Lpm { value: bytes.to_vec().convert(key_name, key_width)?, prefix_len: prefix_length.to_owned() })
                    }
                    MatchValue::Ternary { value, mask} => {
                        MatchType::Ternary(key_field::Ternary{ value: value.to_vec().convert(key_name, key_width)?, mask: mask.to_vec().convert(key_name, key_width)?})
                    }
                };

                let field = KeyField { field_id: key.id(), match_type: Some(match_type) };
    ...
    fn build_table_action_data(&self, request: &Request) -> Result<Option<TableData>, RBFRTError> {
        ...
        let id = {
            for entry in request.get_action_data() {
                let (field_width, field_type, field_id) = { // get field width and whether its a singleton or not
                    ...
                    } else { // it should be a regular action data field
                        let action = self.get_action_by_name(request.get_action_name())?;
                        let action_data = action.get_action_data_by_name(entry.get_name())?;
                        let t = action.get_action_data_type(entry.get_name()).unwrap().clone();
                        (t.get_width(), t.r#type.as_str().to_owned(), action_data.id())
                    }
                };
                ...
                let value = match field_type.as_str() {
                    ...
                    _ => data_field::Value::Stream(entry.get_data().clone().convert(entry.get_name(), field_width)?)
                };

                fields.push(DataField { field_id: field_id, value: Some(value) });
            }

            // do the same for repeated action data fields
            for entry in request.get_action_data_repeated() {
                let (_field_width, field_type, field_id) = { // get field width and whether its a singleton or not
                    let singleton = self.get_singleton_by_name(entry.get_name());
                ...
                fields.push(DataField { field_id, value: Some(value) });
            }

            if request.has_action() {
                self.get_action_by_name(request.get_action_name()).unwrap().id
            }
            else {
                0 // default action id
            }
    ...
    pub fn build_write_request(
        ...
        let update = Update { r#type: mode, entity: Some(ent) };
    ...
    pub fn build_operation_request(
        ...
        let update = Update { r#type: 1, entity: Some(ent) };
    ...
    pub fn build_delete_request(
        ...
        let update = Update { r#type: 4, entity: Some(ent) };

        Ok(update)
    }

    pub(crate) fn parse_read_request(&self, entry: entity::Entity, table_name: &str) -> Result<TableEntry, RBFRTError> {
                ...
                return Ok(TableEntry {
                                        ...
                                        for k in &keys.fields {
                                            ...
                                            match_keys.insert(key.name().to_owned(),
                                                              match k.match_type.as_ref().unwrap() {
                                                                  ...
                                                                  MatchType::Ternary(t) => MatchValue::Ternary { value: (t.value.clone()), mask: (t.mask) }
                    ...
                    action_data: {
                        ...
                        for f in &data.fields {
                            let key_name = self.get_action_param_name(data.action_id, f.field_id)?;
    ...
    pub fn get_action_by_name(&self, name: &str) -> Result<&BFRTAction, RBFRTError> {
        ...
        Err(UnknownActionName { name: name.to_owned() })
    }
    ...
    pub fn get_singleton_by_name(&self, name: &str) -> Result<&BFRTSingleton, RBFRTError> {
        ...
        Err(UnknownSingletonName { name: name.to_owned() })
    }
    ...
    pub fn get_action_param_name(&self, action_id: u32, param_field_id: u32) -> Result<&str, RBFRTError> {
        match self.table_type {
            TableType::MatchActionDirect => {
                ...
                match action_data {
                    ...
                    Err(e) => {
                        match self.get_singleton_by_id(param_field_id) {
                            Ok(s) => Ok(s.name()),
                            Err(_) => Err(e)?
                        }
                    }
    ...
    pub fn get_key_by_id(&self, key_id: u32) -> Result<&BFRTTableKeyObject, RBFRTError> {
        ...
        Err(UnknownKeyId {id: key_id, table_name: self.name.to_owned()})
    }
    ...
    pub fn get_key_by_name(&self, name: &str) -> Result<&BFRTTableKeyObject, RBFRTError> {
        ...
        Err(UnknownKeyName {name: name.to_owned(), table_name: self.name.to_owned()})
    }
